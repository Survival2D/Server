...
let matchLeave: nkruntime.MatchLeaveFunction = function (
  ...
  let gameState = state as GameState;
  for (let presence of presences) {
    let playerNumber: number = getPlayerNumber(
      gameState.players,
      presence.sessionId
    );
  ...
  if (getPlayersCount(gameState.players) == 0) return null;
...
function processMessages(
  ...
  for (let message of messages) {
    ...
    else messagesDefaultLogic(message, gameState, dispatcher);
  }
}

function messagesDefaultLogic(
...
function matchLoopBattle(
  ...
  if (gameState.countdown > 0) {
    ...
    if (gameState.countdown == 0) {
      ...
      dispatcher.broadcastMessage(
        OperationCode.ChangeScene,
        JSON.stringify(gameState.scene)
      );
    }
  }
}
  ...
  if (gameState.countdown > 0) {
    ...
    if (gameState.countdown == 0) {
      ...
      if (winner != null) {
        ...
        let result: nkruntime.StorageObject[] =
          nakama.storageRead(storageReadRequests);
...
function getPlayersCount(players: Player[]): number {
  ...
  for (let playerNumber = 0; playerNumber < MaxPlayers; playerNumber++)
    if (players[playerNumber] != undefined) count++;
...
function playerObtainedNecessaryWins(playersWins: number[]): boolean {
  for (let playerNumber = 0; playerNumber < MaxPlayers; playerNumber++)
    if (playersWins[playerNumber] == NecessaryWins) return true;
...
function getNextPlayerNumber(players: Player[]): number {
  for (let playerNumber = 0; playerNumber < MaxPlayers; playerNumber++)
    if (!playerNumberIsUsed(players, playerNumber)) return playerNumber;
