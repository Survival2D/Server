...
import static survival2d.game.PlayerLogic.velocity;

import survival2d.game.PlayerLogic;
...
public class GamePlayServiceImpl extends EzyLoggable implements GamePlayService {
  ...
  public void handlePlayerInputData(
      String playerName, PlayerInputData inputData, float[] nextRotation) {
        ...
        playerSpawnData -> {
          ...
          synchronized (player) {
            ...
            SortedMap<Integer, Vec3> playerPositionHistory =
                globalPlayersPositionHistory.get(player.getName());
        ...
        playerName -> {
          if (!globalPlayersPositionHistory.containsKey(playerName)) {
            globalPlayersPositionHistory.put(
                playerName, Collections.synchronizedSortedMap(new TreeMap<>()));
          } else {
